...
| <a name="input_override_nlb_resource_names"></a> [override\_nlb\_resource\_names](#input\_override\_nlb\_resource\_names) | Flag to disable appending the zone suffix to NLB resource names (service, deployment, HPA, and PDB). Only allowed when ingress\_nlb\_zones\_subnets contains at most one zone. Default to false. | `bool` | `false` | no |
