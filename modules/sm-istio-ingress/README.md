...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_ingress_namespace"></a> [ingress\_namespace](#module\_ingress\_namespace) | terraform-ibm-modules/namespace/ibm | v2.0.2 |
...
| <a name="input_extend_selectors"></a> [extend\_selectors](#input\_extend\_selectors) | Set to true to add deploymentName label to pods and deploymentSelector to matchLabels, ensuring HPA and PDB work correctly for NLBs. Keep false for ALB. | `bool` | `false` | no |
...
| <a name="input_ingress_autoscale_configuration"></a> [ingress\_autoscale\_configuration](#input\_ingress\_autoscale\_configuration) | Ingress autoscale configuration defined through HPA. If enabled is set to true the HPA definition is deployed. Otherwise if false the HPA configuration is not deployed. The hpa\_name field allows customizing the HPA resource name (defaults to the ingress name if null). Default to enabled=false. | <pre>object({<br/>    enabled : optional(bool, false),<br/>    hpa_name : optional(string, null),<br/>    autoscaleMin : optional(number, 1),<br/>    autoscaleMax : optional(number, 5),<br/>    cpu : optional(object(<br/>      {<br/>        targetavgutil : optional(number, 80)<br/>      }<br/>    ))<br/>    memory : optional(object(<br/>      {<br/>        targetavgutil : optional(number, 80)<br/>      }<br/>    ))<br/>  })</pre> | <pre>{<br/>  "enabled": false<br/>}</pre> | no |
| <a name="input_ingress_create_deployment"></a> [ingress\_create\_deployment](#input\_ingress\_create\_deployment) | Flag to disable ingress Deployment resource creation. Default to true | `bool` | `true` | no |
| <a name="input_ingress_create_service"></a> [ingress\_create\_service](#input\_ingress\_create\_service) | Flag to disable ingress Service resource creation. Default to true | `bool` | `true` | no |
| <a name="input_ingress_create_service_account"></a> [ingress\_create\_service\_account](#input\_ingress\_create\_service\_account) | Flag to disable ingress ServiceAccount resource creation. Default to true | `bool` | `true` | no |
...
| <a name="input_ingress_deployment_custom_annotations"></a> [ingress\_deployment\_custom\_annotations](#input\_ingress\_deployment\_custom\_annotations) | Map of custom annotations to add to the ingress Deployment resource | `map(string)` | `{}` | no |
| <a name="input_ingress_deployment_custom_labels"></a> [ingress\_deployment\_custom\_labels](#input\_ingress\_deployment\_custom\_labels) | Llabel that defines an additional identity for the ingress gateway.<br/>This label is applied to:<br/>  - Deployment metadata.labels | `map(string)` | `{}` | no |
...
| <a name="input_ingress_proxy_protocol_envoy_filter_name"></a> [ingress\_proxy\_protocol\_envoy\_filter\_name](#input\_ingress\_proxy\_protocol\_envoy\_filter\_name) | Optional name override for the EnvoyFilter resource used for Proxy Protocol. If null or empty, defaults to the ingress name. | `string` | `null` | no |
...
| <a name="input_istio_ingress_deployment_timeout"></a> [istio\_ingress\_deployment\_timeout](#input\_istio\_ingress\_deployment\_timeout) | Timeout for the helm release deployment for the ingress gateway. Default to 180s | `number` | `180` | no |
...
| <a name="input_nlb_append_zone_to_resource_names"></a> [nlb\_append\_zone\_to\_resource\_names](#input\_nlb\_append\_zone\_to\_resource\_names) | Flag to append the zone suffix to NLB resource names (service, deployment, HPA, and PDB). If set to false, suffix is not appended. Setting to false is only allowed when ingress\_nlb\_zones\_subnets contains at most one zone. Default to true. | `bool` | `true` | no |
| <a name="input_prefix"></a> [prefix](#input\_prefix) | Prefix value to append to the name of the resources. The name of the ingress resources created with this module will be in format of <prefix>-<name>. If individual resource names for service, deployment or serviceAccount are provided then only they are used directly without appending any prefix. | `string` | `null` | no |
