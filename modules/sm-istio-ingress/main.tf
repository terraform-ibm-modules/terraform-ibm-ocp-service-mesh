...
resource "helm_release" "istio_ingress" {
  ...
  set = [
    ...
    {
      name  = "ingress.overrideNlbResourceNames"
      value = var.override_nlb_resource_names
    },
  ]
...
resource "null_resource" "confirm_ingress_operational_nlb" {
  ...
  provisioner "local-exec" {
    command     = "${path.module}/scripts/confirm-ingress-operational.sh \"${var.namespace}\" \"${var.override_nlb_resource_names ? local.ingress_service_name : "${local.ingress_service_name}-${each.value}"}\" \"nlb\""
    interpreter = ["/bin/bash", "-c"]
    environment = {
      KUBECONFIG = data.ibm_container_cluster_config.cluster_config.config_file_path
    }
...
locals {
  ...
  ingress_services_map = var.ingress_create_service ? (
    var.ingress_loadbalancer_type == "nlb" ? {
      ...
      (var.override_nlb_resource_names ? local.ingress_service_name : "${local.ingress_service_name}-${zone}") => {
        namespace = var.namespace
        service   = var.override_nlb_resource_names ? local.ingress_service_name : "${local.ingress_service_name}-${zone}"
      }
      } : {
