...
variable "istio_controlplane_deployment_timeout" {
  type        = number
  default     = 300
  description = "Deployment timeout in seconds for the istio controlplane. Default to 300 seconds."
  nullable    = false
}
...
variable "istiod_pdb_configuration" {
  description = "Configuration of the PodDisruptionBudget for the istiod (control plane) deployment definition. Default to null to leverage on Istio default configuration."
  default     = null
  type = object({
  ...
  validation {
    ...
    error_message = "Only one of minAvailable and maxUnavailable for var.istiod_pdb_configuration can be set to a value not null"
  }
  validation {
    ...
    error_message = "minAvailable for var.istiod_pdb_configuration value is not valid. It can be set only to a number or percentage (regex ^([0-9]{1,3}%?)$)"
  }
  validation {
    ...
    error_message = "maxUnavailable for var.istiod_pdb_configuration value is not valid. It can be set only to a number or percentage (regex ^([0-9]{1,3}%?)$)"
  }
  validation {
...
variable "mesh_config_trust_domain" {
  type        = string
  default     = null
  description = "The trust domain root of the mesh. All workload certificates in the mesh are issued for this trust domain, following SPIFFE (https://github.com/spiffe/spiffe/blob/main/standards/SPIFFE-ID.md#21-trust-domain) format. When left to the default null value, the Istio default cluster.local is used. For more details: https://github.com/istio-ecosystem/sail-operator/blob/main/docs/api-reference/sailoperator.io.md#meshconfig"
}
