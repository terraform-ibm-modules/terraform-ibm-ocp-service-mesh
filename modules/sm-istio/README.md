...
| <a name="input_is_ambient_mode"></a> [is\_ambient\_mode](#input\_is\_ambient\_mode) | Enable Istio ambient mode. Ambient mode is a sidecarless approach for service mesh that uses a shared node proxy instead of per-pod sidecars. | `bool` | `false` | no |
| <a name="input_istio_controlplane_deployment_timeout"></a> [istio\_controlplane\_deployment\_timeout](#input\_istio\_controlplane\_deployment\_timeout) | Deployment timeout in seconds for the istio controlplane. Default to 300 seconds. | `number` | `300` | no |
...
| <a name="input_istiod_pdb_configuration"></a> [istiod\_pdb\_configuration](#input\_istiod\_pdb\_configuration) | Configuration of the PodDisruptionBudget for the istiod (control plane) deployment definition. Default to null to leverage on Istio default configuration. | <pre>object({<br/>    name           = optional(string, null)<br/>    minAvailable   = optional(string, null)<br/>    maxUnavailable = optional(string, null)<br/>  })</pre> | `null` | no |
...
| <a name="input_mesh_config_enable_prometheus_merge"></a> [mesh\_config\_enable\_prometheus\_merge](#input\_mesh\_config\_enable\_prometheus\_merge) | Enable Istio agent to merge metrics exposed by the application with metrics from Envoy and Istio agent. When null, uses Istio default. For more details: https://github.com/istio-ecosystem/sail-operator/blob/main/docs/api-reference/sailoperator.io.md | `bool` | `null` | no |
...
| <a name="input_mesh_config_extra_stat_tags"></a> [mesh\_config\_extra\_stat\_tags](#input\_mesh\_config\_extra\_stat\_tags) | Additional stat tags to add to Envoy metrics. When null, no extra tags are added. Common tags include: request\_protocol, response\_code, connection\_security\_policy. For more details: https://github.com/istio-ecosystem/sail-operator/blob/main/docs/api-reference/sailoperator.io.md | `list(string)` | `null` | no |
...
| <a name="input_mesh_config_proxy_stats_matcher"></a> [mesh\_config\_proxy\_stats\_matcher](#input\_mesh\_config\_proxy\_stats\_matcher) | Configure which stats to generate for Envoy proxies. When null, no configuration is applied. Set inclusionRegexps to control which metrics are collected (e.g., ['.*'] for all stats). For more details: https://github.com/istio-ecosystem/sail-operator/blob/main/docs/api-reference/sailoperator.io.md | <pre>object({<br/>    inclusionRegexps : optional(list(string), null)<br/>  })</pre> | `null` | no |
| <a name="input_mesh_config_status_port"></a> [mesh\_config\_status\_port](#input\_mesh\_config\_status\_port) | Port on which the Envoy health check, readiness probe, and Prometheus metrics are exposed. When null, uses Istio default (15020). For more details: https://github.com/istio-ecosystem/sail-operator/blob/main/docs/api-reference/sailoperator.io.md | `number` | `null` | no |
...
| <a name="input_mesh_config_trust_domain"></a> [mesh\_config\_trust\_domain](#input\_mesh\_config\_trust\_domain) | The trust domain root of the mesh. All workload certificates in the mesh are issued for this trust domain, following SPIFFE (https://github.com/spiffe/spiffe/blob/main/standards/SPIFFE-ID.md#21-trust-domain) format. When left to the default null value, the Istio default cluster.local is used. For more details: https://github.com/istio-ecosystem/sail-operator/blob/main/docs/api-reference/sailoperator.io.md#meshconfig | `string` | `null` | no |
| <a name="input_mesh_config_trust_domain_aliases"></a> [mesh\_config\_trust\_domain\_aliases](#input\_mesh\_config\_trust\_domain\_aliases) | A list of additional trust domains that to trust as the primary trust domain. Workloads from these trust domains are authenticated and their SPIFFE identities are accepted. When left to the default null value, no aliases are configured. For more details: https://github.com/istio-ecosystem/sail-operator/blob/main/docs/api-reference/sailoperator.io.md#meshconfig | `list(string)` | `null` | no |
...
| <a name="input_priority_class_name"></a> [priority\_class\_name](#input\_priority\_class\_name) | Specifies the Kubernetes priorityClassName for the Istio control plane components. Maps to spec.values.global.priorityClassName. When null, no priorityClassName is set and the Istio default is used. See https://kubernetes.io/docs/concepts/configuration/pod-priority-preemption/#priorityclass and https://github.com/istio-ecosystem/sail-operator/blob/main/docs/api-reference/sailoperator.io.md#globalconfig | `string` | `null` | no |
...
| <a name="input_ztunnel_namespace"></a> [ztunnel\_namespace](#input\_ztunnel\_namespace) | Namespace for the ztunnel component when ambient mode is enabled. Only used when is\_ambient\_mode is true. | `string` | `"ztunnel"` | no |
