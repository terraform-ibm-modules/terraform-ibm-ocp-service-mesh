...
| <a name="input_enable_dns_capture"></a> [enable\_dns\_capture](#input\_enable\_dns\_capture) | Enable DNS capture for ServiceEntry resources. Required for ServiceEntry resources that rely on DNS resolution. In OpenShift Service Mesh 3.0, DNS capture is disabled by default (aligned with upstream Istio). Set to true to enable DNS capture by setting ISTIO\_META\_DNS\_AUTO\_ALLOCATE and ISTIO\_META\_DNS\_CAPTURE to true in proxy metadata. For more details refer to https://docs.redhat.com/en/documentation/red_hat_openshift_service_mesh/3.0/html-single/migrating_from_service_mesh_2_to_service_mesh_3/index#ossm-migrating-read-me-dns-capture-configuration_ossm-migrating-read-me | `bool` | `false` | no |
