...
| <a name="input_istiod_pdb_configuration"></a> [istiod\_pdb\_configuration](#input\_istiod\_pdb\_configuration) | Custom PodDisruptionBudget configuration for the istiod (control plane) deployment. When set, a dedicated PodDisruptionBudget is created for istiod pods (labels: app=istiod, istio=pilot). Set to null to rely solely on the default PDB controlled by var.istio\_enable\_default\_pod\_disruption\_budget. Only one of minAvailable or maxUnavailable may be set. | <pre>object({<br/>    name           = optional(string, null)<br/>    minAvailable   = optional(string, null)<br/>    maxUnavailable = optional(string, null)<br/>  })</pre> | `null` | no |
