...
| <a name="input_proxy_metadata"></a> [proxy\_metadata](#input\_proxy\_metadata) | Additional key-value pairs to configure meshConfig.defaultConfig.proxyMetadata. Use this to add custom proxy metadata like HTTP\_PROXY, HTTPS\_PROXY, etc. Do not include ISTIO\_META\_DNS\_AUTO\_ALLOCATE or ISTIO\_META\_DNS\_CAPTURE here; use the enable\_dns\_capture variable instead. | `map(string)` | `{}` | no |
